...
def init_database():
    ...
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_activity_platform ON activity_logs(platform)"
    )
    cursor.execute(
    ...
    # Channel publications table - tracks which channels shorts should be published to
    cursor.execute(
        """
    ...
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_sessions_filename ON sessions_metadata(filename)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_sessions_role ON sessions_metadata(role)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_sessions_type ON sessions_metadata(session_type)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_sessions_date ON sessions_metadata(session_date)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_session_content_filename ON session_content(session_filename)"
    )
    cursor.execute(
    ...
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_playlists_role ON playlists(playlist_role)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_settings_key ON settings(setting_key)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_channel_pub_video_id ON channel_publications(video_id)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_channel_pub_target ON channel_publications(target_channel)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_channel_pub_status ON channel_publications(publication_status)"
    )
...
def insert_or_update_social_post(
    video_id: str, platform: str, post_data: Dict[str, Any]
) -> int:
...
def get_videos_by_playlist(playlist_id: str) -> List[Dict[str, Any]]:
    ...
    cursor.execute(
        "SELECT * FROM videos WHERE playlist_id = ? ORDER BY created_at DESC",
        (playlist_id,),
    )
...
def get_activity_logs(
    limit: int = 100, platform: Optional[str] = None, action_type: Optional[str] = None
) -> List[Dict[str, Any]]:
...
def export_to_excel(output_path: str, playlist_id: Optional[str] = None):
    ...
    if playlist_id:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT DISTINCT playlist_id, playlist_name FROM videos ORDER BY playlist_name"
        )
...
def save_settings_to_db(settings: Dict[str, Any]) -> None:
    ...
    try:
        ...
        print(
            f"✅ Settings saved to database successfully (size: {len(settings_json)} bytes)"
        )
    ...
    try:
        ...
        if result and result["setting_value"]:
            try:
                ...
                updated_at = (
                    result["updated_at"] if "updated_at" in result.keys() else "unknown"
                )
